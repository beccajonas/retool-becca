WITH AverageDeltas AS (
  SELECT
    DATE(execution_date) as datum, competitor, cheapest_comp_supplier,
...
RollingAggregations AS (
  SELECT 
    datum, competitor, cheapest_comp_supplier,
  ...
  WINDOW w AS (PARTITION BY competitor, cheapest_comp_supplier
...
SELECT *, urgency_score - LAG(urgency_score, 7, 0) OVER (PARTITION BY competitor, cheapest_comp_supplier ORDER BY datum) as trend
FROM UrgencyScores
