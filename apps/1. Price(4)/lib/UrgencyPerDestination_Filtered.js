...
let urgencyScores = {{UrgencyPerDestination.data}}
let selectedCompetitor = {{Competitor_Selection.value}}
let selectedDestination = {{UrgencyTable_Destination.selectedRow.destination}}
let datum = []
let win_rate = []
let low_win_rate = []
let parity_rate = []
...
for (let i = 0; i < urgencyScores.datum.length; i++) {
  if (urgencyScores.competitor[i] == selectedCompetitor && urgencyScores.destination[i] == selectedDestination) {
